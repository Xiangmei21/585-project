...
tidy = dat %>% gather(key=education,value=value,8:ncol(dat)) %>% 
  ...
  mutate(year=trimws(year),FIPS=parse_number(FIPS),
         type="count",
...
wide %>% filter(FIPS==5055,year=="1970") %>% mutate(new_p=count/sum(count)*100)
wide %>% filter(FIPS==26015,year=="1970") %>% mutate(new_p=count/sum(count)*100)
wide %>% filter(FIPS==38083,year=="2000") %>% mutate(new_p=count/sum(count)*100) ## differences due to rounding

wide %>% filter(FIPS==72000,year=="2000") %>% mutate(new_c=sum(count)*percent/100) ## percentage are wrong!!
# correct the percentage
wide[wide$FIPS==72000 & wide$year=="2000","percent"]= wide %>% filter(FIPS==72000,year==2000) %>% 
   mutate(new_p=round(count/sum(count)*100,1)) %>% select(new_p)

tidy=wide %>% gather(key=type,value=value,6:7)
# visualize
tidy%>%filter(State=="IA",Area=="Story County") %>%
ggplot(aes(x=year,y=value))+
  geom_point(aes(color=level)) + geom_path(aes(group=level,color=level)) + 
  facet_wrap(~type,scale="free") + 
  theme_bw() + theme(legend.position = "bottom") + ggtitle("Story County, IA") +
...
select=isstate %>% filter(type=="percent",level=="High school diploma only")
all=left_join(select,map,by=c("state" = "region"))

all %>% ggplot(aes( x = long, y = lat, group = state)) +
  geom_polygon(aes(fill = value))  + 
  scale_fill_gradient2(name = "Percent", midpoint = min(all$value), mid = "white") +
  ggthemes::theme_map() + theme(legend.position = c(0.8,0))+ facet_wrap( ~year) +
  ggtitle("High school diploma only")
